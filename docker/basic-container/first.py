...
def child_func(stack):
    ...
    os.system(
        f"mount -t overlay -o lowerdir={BASE_DIR},upperdir={UPPER_DIR},workdir={WORK_DIR} overlay {ROOT_FS}")
    create_memory_group()
    set_hostname("front")
...
def run():
    ...
    pid = libc.clone(child_func_c, stack, CLONE_NEWNS | CLONE_NEWPID |
                     CLONE_NEWUTS | CLONE_NEWCGROUP | CLONE_NEWNET | 17)
